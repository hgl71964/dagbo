...
class DelayedSparkDagInit:
    def __call__(self, train_input_names, train_target_names, train_inputs,
                 train_targets, num_samples):
        return SparkDag(train_input_names, train_target_names, train_inputs,
                        train_targets, num_samples)
...
# SEARCH SPACE
executor_cores = RangeParameter("spark.executor.cores",
                                ParameterType.INT,
                                lower=1,
                                upper=8)
executor_memory = RangeParameter("spark.executor.memory",
                                 ParameterType.INT,
                                 lower=512,
                                 upper=14336)
task_cpus = RangeParameter("spark.task.cpus",
                           ParameterType.INT,
                           lower=1,
                           upper=8)
memory_fraction = RangeParameter("spark.memory.fraction",
                                 ParameterType.FLOAT,
                                 lower=0.01,
                                 upper=0.99)
shuffle_compress = ChoiceParameter("spark.shuffle.compress",
                                   ParameterType.BOOL,
                                   values=[False, True],
                                   is_ordered=True)
shuffle_spill_compress = ChoiceParameter("spark.shuffle.spill.compress",
                                         ParameterType.BOOL,
                                         values=[False, True],
                                         is_ordered=True)
# order of parameters (configurable variables) is arbitrary be must be consistent throughout program
parameters = [
    executor_cores, executor_memory, task_cpus, memory_fraction,
    shuffle_compress, shuffle_spill_compress
]

task_cpus_lt_executor_cores = ParameterConstraint(constraint_dict={
    "spark.task.cpus": 1.,
...
simple_exp = SimpleExperiment(search_space=search_space,
...
for _ in range(num_trials):
    # model fitting
    model = get_botorch(search_space=simple_exp.search_space,
